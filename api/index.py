...
async def get_ai_analysis(prompt: str, context: str = "") -> str:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.debug(f"🎯 Sending to Gemini 2.5 Flash model (Attempt {attempt + 1}/{max_retries})...")
            ...
            # Check for rate limits or overloaded server (429 / 503)
            if "503" in error_str or "429" in error_str or "UNAVAILABLE" in error_str or "high demand" in error_str.lower():
                if attempt < max_retries - 1:
                    delay = base_delay * (2 ** attempt)  # Wait 2s, then 4s before trying again
                    logger.warning(f"⚠️ Model overloaded (503/429). Retrying in {delay} seconds...")
                    await asyncio.sleep(delay)
                    continue
                else:
                    logger.error("❌ AI analysis failed after multiple retries due to high demand.")
